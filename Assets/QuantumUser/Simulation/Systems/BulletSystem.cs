namespace Quantum
{
    ...
    public unsafe class BulletSystem : SystemMainThreadFilter<BulletSystem.Filter>, ISignalCreateBullet
    {
        ...
        private bool CheckForCollisions(Frame f, Filter filter, FPVector2 nextPosition, out EntityRef entityHit)
        {
            entityHit = EntityRef.None;

            var owner = filter.Bullet->Owner;
            var bulletPosition = filter.Transform->Position;
            var collisions = f.Physics2D.LinecastAll(bulletPosition, bulletPosition + nextPosition, int.MaxValue, QueryOptions.HitAll &~ QueryOptions.HitTriggers);
        ...
        public void CreateBullet(Frame f, EntityRef owner, FiringWeaponAsset weaponData)
        {
